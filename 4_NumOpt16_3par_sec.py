...
def jacb(t1):
    der=-a_16*(np.exp(-(b_16*t1))+np.exp(-d_16*t1))
    #result=np.sum(der)
    return der
...
ax1.set_ylabel('Normalized Frequencies')
ax1.set_title('2016')
ax1.plot([],[], "k.", label=r'Initial $L_{\mathrm{tot}}$='+'{:.2f}'.format(L_tot_2016_3Par/1e9)+r' [$\mathrm{fb}^{-1}$]')
...
with open('Data/L_tot_2016_3Par.txt', 'w') as f:
        f.write('')
