...
L_int_2018_4Par=np.array(L_int_2018)
L_tot_2018_4Par=np.sum(L_int_2018_4Par)
...
with open('Data/res_2018_4Par.txt', 'w') as f:
        f.write('')
        f.close()
for el in res.x:
    with open('Data/res_2018_4Par.txt', 'a') as f:
        f.write(str(el))
        f.write('\n')
...
ax1.hist(x0/3600, facecolor="pink", alpha=0.8,density=True,  label="Real Fill Times" )
...
ax1.set_title('2018')
plt.legend(loc='best')
plt.savefig('OptimizationResults/2018_4Par.pdf')
plt.show()

#comparison between real and optimized integrated luminosity
fig, ax1= plt.subplots()
ax1.hist(L_int_2018_4Par/1e9,  facecolor="pink", alpha=0.8, density=True,  label="Real Integrated Luminosities" )
...
ax1.set_ylabel('Normalized Frequencies')
ax1.set_title('2018')
ax1.plot([],[], "k.", label=r'Initial $L_{\mathrm{tot}}$='+'{:.2f}'.format(L_tot_2018_4Par/1e9)+r' [$\mathrm{fb}^{-1}$]')
ax1.plot([],[], "k.", label=r'Optimized $L_{\mathrm{tot}}$='+'{:.2f}'.format(L_tot_opt/1e9)+r' [$\mathrm{fb}^{-1}$]')
plt.legend(loc='upper left')
plt.savefig('OptimizationResults/2018_4Par_lumi.pdf')
plt.show()
...
fig, ax1= plt.subplots()
L_ist=fun2(res.x)
ax1.hist(L_ist,  facecolor='pink', density=True, alpha=0.8)
ax1.set_xlabel(r'Istantaneous Luminosities [Hz/$\mu$b]')
ax1.set_ylabel('Normalized Frequencies')
ax1.set_title('2018')
plt.savefig('OptimizationResults/2018_4Par_istLumi.pdf')
plt.show()


#comparison between real and optimized integrated luminosity
fig, ax1= plt.subplots()
ax1.hist(L_int_2018_4Par/1e9,  facecolor="green", alpha=0.3, density=True, label="Real Integrated Luminosities" )
ax1.hist(L_int_summary_18/1e9, histtype='step', density=True, label="Measured Integrated Luminosity")
ax1.set_xlabel(r'Integrated Luminosity [$\mathrm{fb}^{-1}$]')
ax1.set_ylabel('Normalized Frequencies')
ax1.set_title('2018')
plt.legend(loc='upper left')
plt.savefig('OptimizationResults/2018_4Par_Real_Measured_Lumi.pdf')
plt.show()

with open('Data/L_tot_2018_4Par.txt', 'w') as f:
        f.write('')
