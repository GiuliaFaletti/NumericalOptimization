...
L_int_2018_3Par=np.array(L_int_2018)
L_tot_2018_3Par=np.sum(L_int_2018_3Par)

#Objective Function
def fun(t1):
    result=np.empty(len(x0))
    for i in range(len(x0)):
        lam=lambda x1: a_18[i]*(np.exp(-(b_18[i]*x1))+np.exp(-d_18[i]*x1))
...
def jacb(t1):
    der=-a_18*(np.exp(-(b_18*t1))+np.exp(-d_18*t1))
    #result=np.sum(der)
    return der
...
ax1.set_title('2018')
plt.legend(loc='best')
plt.savefig('OptimizationResults/2018_3Par.pdf')
plt.show()

#comparison between real and optimized integrated luminosity
fig, ax1= plt.subplots()
ax1.hist(L_int_2018_3Par/1e9,  facecolor='steelblue', density=True, alpha=0.4, label="Real Integrated Luminosities" )
...
ax1.set_ylabel('Normalized Frequencies')
ax1.set_title('2018')
ax1.plot([],[], "k.", label=r'Initial $L_{\mathrm{tot}}$='+'{:.2f}'.format(L_tot_2018_3Par/1e9)+r' [$\mathrm{fb}^{-1}$]')
ax1.plot([],[], "k.", label=r'Optimized $L_{\mathrm{tot}}$='+'{:.2f}'.format(L_tot_opt/1e9)+r' [$\mathrm{fb}^{-1}$]')
plt.legend(loc='upper left')
plt.savefig('OptimizationResults/2018_3Par_lumi.pdf')
plt.show()
...
def fun2(t1):
    result=np.empty(len(x0))
    for i in range(len(x0)):
        result[i]=a_18[i]*(np.exp(-(b_18[i]*t1[i]))+np.exp(-d_18[i]*t1[i]))
    return result
...
ax1.set_ylabel('Normalized Frequencies')
ax1.set_title('2018')
plt.savefig('OptimizationResults/2018_3Par_istLumi.pdf')
plt.show()
...
#comparison between real and optimized integrated luminosity
fig, ax1= plt.subplots()
ax1.hist(L_int_2018_3Par/1e9,  facecolor="green", alpha=0.3, density=True,label="Real Integrated Luminosities" )
ax1.hist(L_int_summary_18/1e9, histtype='step', density=True, label="Measured Integrated Luminosity")
ax1.set_xlabel(r'Integrated Luminosity [$\mathrm{fb}^{-1}$]')
ax1.set_ylabel('Normalized Frequencies')
ax1.set_title('2018')
plt.legend(loc='upper left')
plt.savefig('OptimizationResults/2018_3Par_Real_Measured_Lumi.pdf')
plt.show()

with open('Data/L_tot_2018_3Par.txt', 'w') as f:
        f.write('')
